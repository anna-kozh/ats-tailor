...
function scoreResume(resume, jdKeywords, meta) {
...
async function callOpenAIJSON(apiKey, model, systemPrompt, userPrompt, schema, temperature = 0.2, max_tokens = 900, schemaName = "schema") {
  const body = {
    model,
...
Return JSON exactly in this shape: {"keywords":["..."]} Only JSON, no extra text.`;
    ...
    // --- PASS 1b: PLACEMENT PLANNER ---
    const sections = parseSections(masterInventory);
    const plannerSystem = `Map JD keywords to the candidate's existing WORK EXPERIENCE bullets.
Rules:
- Prefer mapping to an existing bullet in WORK EXPERIENCE when evidence exists.
- Only map to Summary/Skills if no suitable bullet.
- If no evidence anywhere, mark as "skip".
- Never invent facts.`;
...
{ "placements": [
  { "keyword": "string", "target": "experience|summary|skills|skip", "bullet_index": number|null, "evidence": "exact phrase from candidate text (or empty)" }
    ...
    const placementPlan = await callOpenAIJSON(apiKey, 'gpt-4o', plannerSystem, plannerUser, planSchema, 0.2, 900, 'placement_plan');
    ...
    const bulletSystem = `Edit only WORK EXPERIENCE bullets, in place.
Rules:
...
- If a keyword maps to summary/skills, put it in the corresponding *_additions arrays (once each); do not touch bullets for those.
- Australian spelling. Leadership tone. Active voice.
...
${masterInventory}`;
    ...
    const bulletEdits = await callOpenAIJSON(apiKey, 'gpt-4o', bulletSystem, bulletUser, bulletSchema, 0.2, 900, 'bullet_rewriter');
    ...
    const jdKeywords = keywords.map(k => ({ term: k, type: 'hard' }));
